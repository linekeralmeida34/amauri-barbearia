...
export const BookingFlow = () => {
  ...
  const [services, setServices] = useState<LocalService[]>(fallbackServices as LocalService[]);
  ...
  useEffect(() => {
    ...
    } else if (isTodayLocal(selectedDate) && selectedTime && !hmGte(selectedTime, nowLocalHM())) {
  ...
  useEffect(() => {
    ...
    listAvailableTimes(String(selectedBarber.id), selectedDate, selectedService.duration)
      .then((serverSlots) => {
        const nowHM = nowLocalHM();
  ...
  }, [selectedBarber, selectedService, selectedDate]); // eslint-disable-line react-hooks/exhaustive-deps
            ...
            <div className="grid md:grid-cols-2 gap-6 md:gap-8">
              <div>
                ...
                <Input
                  type="date"
                  value={selectedDate}
                  min={min}                                // usa HOJE em fuso local (corrige bug de UTC no mobile)
                  onChange={(e) => {
                ...
                />
                {!selectedDate && (
                  <p className="text-xs text-muted-foreground">Selecione uma data para ver horários.</p>
                )}
              </div>
              <div>
                ...
                {!hasBasics ? (
                  <div className="text-sm text-muted-foreground">Escolha o serviço, barbeiro e a data.</div>
                ) : loadingSlots ? (
                    ...
                    {effectiveSlots.map((time) => {
                      ...
                      return (
                        <Button
                          ...
                          disabled={disabled}
                          className={`h-9 ${
                            isSelected ? "bg-barbershop-gold text-barbershop-dark" : ""
